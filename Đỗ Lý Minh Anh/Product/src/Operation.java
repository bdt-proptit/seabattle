...
public class Operation {
    ...
    public Coordinates chooseTheFirstSquare (Scanner sc, Player player, Boat boat) {
        ...
        while (count < location.length()) {
            number += location.charAt(count);
            ++count;
        }
        coordinates.column = Integer.parseInt(number) - 1;
    ...
    public void playerSetsUpTheBoard (Scanner sc, Player player) {
        ...
        if (thePatrolBoat1.getShape() == "Row") {
        ...
        if (thePatrolBoat2.getShape() == "Row") {
        ...
        if (theDestroyerBoat.getShape() == "Row") {
